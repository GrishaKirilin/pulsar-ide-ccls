...
const {AutoLanguageClient, Convert} = require('@savetheclocktower/atom-languageclient')
...
class CCLSLanguageClient extends AutoLanguageClient {
  ...
  startServerProcess (projectPath) {
    ...
    // TODO[andrphi]: Render config depends on version
    var init = config.cclsInit;
  ...
  determineProjectPath(editor) {
    ...
    for (;;) {
      if (fs.existsSync(path.join(directory, '.ccls'))) {
        return directory;
      }
    ...
    return super.determineProjectPath(editor);
  }
}
